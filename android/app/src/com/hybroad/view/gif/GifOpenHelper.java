...
public class GifOpenHelper {
  ...
  private boolean breakDecodeFlag = false;

  public void breakDecode() {
    breakDecodeFlag = true;
  }
  ...
  private Bitmap resizedBitmap(Bitmap bitmap) {
    ...
    // matrix, true);
    //
    // if (resizedBitmap.getWidth() > 320) {
    // matrix.postScale((0.6f * 0.8f), (0.6f * 0.8f));
    // resizedBitmap = Bitmap.createBitmap(bitmap, 0, 0, width, height,
    // matrix, true);
    //
    // if (resizedBitmap.getWidth() > 320) {
    // matrix.postScale((0.6f * 0.6f * 0.6f), (0.6f * 0.6f * 0.6f));
  ...
  public Bitmap nextBitmap() {
    ...
    im = frames.elementAt(frameindex).image;
    return resizedBitmap(im);
  }
  ...
  public int nextDelay() {
    ...
    // DTS2011011804074
    return (frames.size() != 1) ? frames.elementAt(frameindex).delay : 1000;
    // DTS2011011804074
  }
  ...
  protected void readContents() {
    ...
    boolean done = false;
    while (!(done || err() || breakDecodeFlag)) {
      int code = read();
      switch (code) {
  ...
  protected void readImage() {
    /**
     * begin DTS2011092304212 modify by duanqingpeng 2011-11-02
     **/
    try {
      // offset of X
