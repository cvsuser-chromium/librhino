...
{
    if ((int)jerr < 0 || jerr > sizeof(json_tokener_errors))
...
redo_char:
        ...
        case json_tokener_state_start:
            ...
            case '-':
                ...
                tok->is_double = 0;
                goto redo_char;
            case ',': //zp, 2011.4.8   ",startTime:,endTime:,"
        ...
        case json_tokener_state_object_field:
            {
                ...
                while(1) {
                    if (tok->quote_char != '$' && c == ':' ) {
                        ...
                        MC_DEBUG("########[%d]########## object name:%s\n", __LINE__, obj_field_name);
                        saved_state = json_tokener_state_object_field_end;
                        state = json_tokener_state_eatws;
                        MC_DEBUG("##[%d]## \n", __LINE__);
                        goto redo_char;
                    }else if (tok->quote_char != '$' && c == tok->quote_char) {
                        printbuf_memappend_fast(tok->pb, case_start, str-case_start);
                        obj_field_name = strdup(tok->pb->buf);
                        saved_state = json_tokener_state_object_field_end;
                        state = json_tokener_state_eatws;
                        break;
                    } else if(tok->quote_char == '$'){            //zp, 2011.3.15
                        if(c == ' '|| c == '"'){
                            printbuf_memappend_fast(tok->pb, case_start, str-case_start);
                            obj_field_name = strdup(tok->pb->buf);
                            saved_state = json_tokener_state_object_field_end;
