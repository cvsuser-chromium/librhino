...
int avformat_find_stream_info(AVFormatContext *ic, AVDictionary **options)
{
	...
	HT_PRINT("ic->nb_streams = ", ic->nb_streams);
    for(i=0;i<ic->nb_streams;i++) {
        AVCodec *codec;
        st = ic->streams[i];
...
/*            if(!st->time_base.num)
		...
		HT_PRINT("st->codec->codec = ", (int)(st->codec->codec));
	...
	HT_PRINT("more accurate process ", 0);
    count = 0;
    read_size = 0;
    for(;;) {
        if(url_interrupt_cb()){
		...
		if(ic->nb_streams > 0 && st->codec->codec_id == CODEC_ID_PNG)
			ic->probesize = 60000;
...
#if 1
int avformat_find_stream_info_ex(AVFormatContext *ic, AVDictionary **options)
{
	int i, count, ret, read_size, j;
	AVStream *st;
	AVPacket pkt1, *pkt;
	...
	for(;;) 
	{
		...
		for(i=0;i<ic->nb_streams;i++) 
		{
			...
			/* variable fps and no guess at the real fps */
			if(   tb_unreliable(st->codec) && !(st->r_frame_rate.num && st->avg_frame_rate.num)
			&& st->info->duration_count < fps_analyze_framecount	&& st->codec->codec_type == AVMEDIA_TYPE_VIDEO)
				break;
			if(st->parser && st->parser->parser->split && !st->codec->extradata)
				break;
			if(st->first_dts == AV_NOPTS_VALUE)
	...
	for(i=0;i<ic->nb_streams;i++)
	{
		st = ic->streams[i];
		if (st->codec_info_nb_frames>2 && !st->avg_frame_rate.num && st->info->codec_info_duration)
			av_reduce(&st->avg_frame_rate.num, &st->avg_frame_rate.den, (st->codec_info_nb_frames-2)*(int64_t)st->time_base.den, st->info->codec_info_duration*(int64_t)st->time_base.num, 60000);
		...
		if (st->codec->codec_type == AVMEDIA_TYPE_VIDEO) 
		{
			...
			if (st->info->duration_count && !st->r_frame_rate.num && tb_unreliable(st->codec) /*&&
				//FIXME we should not special-case MPEG-2, but this needs testing with non-MPEG-2 ...
				st->time_base.num*duration_sum[i]/st->info->duration_count*101LL > st->time_base.den*/)
			{
				int num = 0;
