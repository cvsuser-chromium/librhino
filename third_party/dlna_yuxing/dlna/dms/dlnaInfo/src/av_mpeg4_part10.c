...
/* MPEG-4 Container */
/********************/
static dlna_profile_t mkv_video={
	.id = NULL,
	.mime = MIME_VIDEO_MKV,
	.label = NULL
};
...
static const struct {
  ...
  //add by lss
  { &avc_mp4_bl_l3_sd_aac, CT_MPEG_TRANSPORT_STREAM,
  ...
  //add by lss
  { &avc_mp4_bl_l12_cif15_heaac, CT_MP4,
    AVC_VIDEO_PROFILE_BL_L12_CIF15, AUDIO_PROFILE_AAC },
                        ...
                        int size, AVCodecContext *vc)
{
  ...
  {
    if (res[i].width == vc->width && res[i].height == vc->height)
      return 1;
	else if (res[i].width == vc->width)
		return 1;
	else if(res[i].height == vc->height)
		return 1;
  }
           ...
           av_codecs_t *codecs)
{
  ...
  if (!stream_ctx_is_av (codecs))
    return NULL;
  if(!strcmp (ctx->iformat->name,"matroska,webm"))
	return &mkv_video;
  ...
  //add by lss
  if(ap == AUDIO_PROFILE_MP2)
	  ap = AUDIO_PROFILE_MP3;
...
dlna_registered_profile_t dlna_profile_av_mpeg4_part10 = {
  ...
  .class = DLNA_CLASS_AV,
  .extensions = "mov,hdmov,mp4,3gp,3gpp,mpg,mpeg,mpe,mp2t,ts,mkv",
  .probe = probe_avc,
  .next = NULL
};
