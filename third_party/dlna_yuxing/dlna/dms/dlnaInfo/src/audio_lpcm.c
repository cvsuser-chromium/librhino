...
static dlna_profile_t audio_wav = {
//	.id = "WAV",
	.id = NULL,
	.mime = MIME_AUDIO_WAV,
	.label = LABEL_AUDIO_2CH
...
audio_profile_t
audio_profile_guess_wav (AVCodecContext *ac)
{
  if (!ac)
    return AUDIO_PROFILE_INVALID;
  ...
  return AUDIO_PROFILE_WAV;
}


static dlna_profile_t *
probe_wav (AVFormatContext *ctx dlna_unused,
            ...
            av_codecs_t *codecs)
{
  ...
  if (audio_profile_guess_wav (codecs->ac) != AUDIO_PROFILE_WAV)
    return NULL;
