...
/* WMA lossless version */
static dlna_profile_t wmalsl = {
  .id = "WMALSL",
  .mime = MIME_AUDIO_WMA,
  .label = LABEL_AUDIO_2CH_MULTI
};
...
audio_profile_guess_wma (AVCodecContext *ac)
{
  ...
  if (ac->codec_id != CODEC_ID_WMAV1 && ac->codec_id != CODEC_ID_WMAV2)
	...
	*/
	...
	if(ac->codec_id == CODEC_ID_WMAPRO)
		return AUDIO_PROFILE_WMA_PRO;
	
	if(ac->codec_id == CODEC_ID_WMALOSSLESS)
		return AUDIO_PROFILE_WMA_LSL;
