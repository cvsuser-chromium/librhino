 ...
 *****************************************************************************/
int DMA_dvbpsi_PushPacket(dvbpsi_handle h_dvbpsi, unsigned char* p_data)
{
  unsigned char i_expected_counter;           /* Expected continuity counter */
  dvbpsi_psi_section_t* p_section;      /* Current section */
  unsigned char* p_payload_pos;               /* Where in the TS packet */
  unsigned char* p_new_pos = NULL;            /* Beginning of the new section,
...
int bRet=0;
  ...
  if(p_data[0] != 0x47)
  {
    DMA_DVBPSI_ERROR("PSI decoder", "not a TS packet");
    return -1;
  }
...
*/
    ...
    {
      DMA_dvbpsi_DeletePSISections(h_dvbpsi->p_current_section);
  ...
  if(p_section == NULL)
  {
    if(p_new_pos)
    {
      ...
      h_dvbpsi->p_current_section
                        = p_section
                        = DMA_dvbpsi_NewPSISection(h_dvbpsi->i_section_max_size);
    ...
    if(i_available >= h_dvbpsi->i_need)
    {
      ...
      if(!h_dvbpsi->b_complete_header)
      {
        ...
        h_dvbpsi->i_need = p_section->i_length
                         =   ((unsigned short)(p_section->p_data[1] & 0xf)) << 8
                           | p_section->p_data[2];
        /* Check that the section isn't too long */
        if(h_dvbpsi->i_need > h_dvbpsi->i_section_max_size - 3)
        {
          DMA_DVBPSI_ERROR("PSI decoder", "PSI section too long");
      ...
      else
      {
        ...
        if(DMA_dvbpsi_ValidPSISection(p_section))
        {
