...
/* $Id: service.c 265 2006-08-27 17:53:14Z r3mi $
 *
 * UPnP Service
...
{
	int rc;
    HT_DBG_FUNC_START(HT_BIT_FEW, HT_MOD_DMC, (int)serv, 0);
	if (serv == NULL) {
...
finalize (Object* obj)
{
	...
	if (serv) {
		// If we have a valid control SID, then unsubscribe
		//(void) Service_UnsubscribeEventURL (serv); yanyongmeng 2011/4/28 , it is dangerous!
 ...
 * Service_UpdateState
 *****************************************************************************/
int Service_UpdateState (Service* serv, IXML_Document* changedVariables, char *udn)
{
  int rc = UPNP_E_SUCCESS;
	...
	if (variables) {
	  ...
	  for (j = 0; j < length1; j++ ) {
		...
		OBJECT_METHOD (serv, update_variable) (serv,
						       var->name, var->value);
          extern void Service_UpdatedCallback(char *dmsUdn, char *name, char *value);
           // if( !strcmp(var->name, "ContainerUpdateIDs")  )
            {
                Service_UpdatedCallback(udn, var->name, var->value);
            }
	    }
	  }
	}
 ...
 *****************************************************************************/
int
Service_SendActionAsyncEx (const Service* serv,
			 ...
			 int nb_params, const StringPair* params)
{
  ...
  if (serv == NULL) {
    ...
    if (actionNode == NULL) {
      ...
      // Send action request
#if UPNP_VERSION > 10666
      rc = UpnpSendActionAsyncEx (serv->ctrlpt_handle, serv->controlURL,
 ...
 *****************************************************************************/
int
Service_SendActionAsyncVaEx (const Service* serv,
			   Upnp_FunPtr callback,
			   void *cookie,
			   const char* actionName, ...)
{
  ...
  return Service_SendActionAsyncEx (serv, callback, cookie, actionName, nb, params);
}
