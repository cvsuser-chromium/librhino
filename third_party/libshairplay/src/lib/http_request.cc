...
on_url(http_parser *parser, const char *at, size_t length)
{
	...
	request->url = (char*)realloc(request->url, urllen+length+1);
	assert(request->url);
...
on_header_field(http_parser *parser, const char *at, size_t length)
{
	...
	if (request->headers_index == request->headers_size) {
		...
		request->headers = (char**)realloc(request->headers,
	...
	if (request->headers[request->headers_index] == NULL) {
		request->headers[request->headers_index] = (char*)calloc(1, length+1);
	} else {
		request->headers[request->headers_index] = (char*)realloc(
...
on_body(http_parser *parser, const char *at, size_t length)
{
	...
	request->data = (char*)realloc(request->data, request->datalen+length);
	assert(request->data);
...
on_message_complete(http_parser *parser)
{
	...
	request->method = http_method_str(http_method(request->parser.method));
	request->complete = 1;
	return 0;
}
