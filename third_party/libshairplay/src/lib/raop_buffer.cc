...
raop_buffer_queue(raop_buffer_t *raop_buffer, unsigned char *data, unsigned short datalen, int use_seqnum)
{
  ...
  unsigned char packetbuf[RAOP_PACKET_LEN];
	...
	shairplay::AES_set_key(&aes_ctx, raop_buffer->aeskey, raop_buffer->aesiv, shairplay::AES_MODE_128);
	shairplay::AES_convert_key(&aes_ctx);
	shairplay::AES_cbc_decrypt(&aes_ctx, &data[12], packetbuf, encryptedlen);
