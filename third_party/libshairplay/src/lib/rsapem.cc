...
rsapem_init(const char *pemstr)
{
	...
	b64dec = shairplay::base64_init(NULL, 0, 1);
	datalen = shairplay::base64_decode(b64dec, &data, pemstr+sizeof(RSAPRIVHEADER),
	                        (footer-header)-sizeof(RSAPRIVHEADER));
	shairplay::base64_destroy(b64dec);
	b64dec = NULL;
	...
	rsapem = (rsapem_t*)calloc(1, sizeof(rsapem_t));
	if (!rsapem) {
		free(data);
		return NULL;
	}
