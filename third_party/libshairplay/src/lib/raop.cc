...
//added by teddy at 2014.03.17 Mon 17:09:29.
static bool
conn_should_terminate(void* ptr)
{
  raop_conn_t *conn = (raop_conn_t *)ptr;
  return false;
}
...
raop_init(int max_clients, raop_callbacks_t *callbacks, const char *pemkey, int *error)
{
	...
	raop = (raop_t *)calloc(1, sizeof(raop_t));
	if (!raop) {
		return NULL;
	}
  ...
  httpd_cbs.conn_should_terminate = &conn_should_terminate;
