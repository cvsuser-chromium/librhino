...
httpd_init(logger_t *logger, httpd_callbacks_t *callbacks, int max_connections)
{
  ...
  httpd = (httpd_t*)calloc(1, sizeof(httpd_t));
  if (!httpd) {
    return NULL;
  }

  httpd->max_connections = max_connections;
  httpd->connections = (http_connection_t*)calloc(max_connections, sizeof(http_connection_t));
...
httpd_add_connection(httpd_t *httpd, int fd, unsigned char *local, int local_len, unsigned char *remote, int remote_len)
{
  ...
  /*
   * added by teddy at 2014.03.17 Mon 18:50:14
   * we only serve for one client.
   * if max_connections equals 1, then we replace the current one.
   */
  if (1 == httpd->max_connections && 1 == httpd->connections[0].connected) {
    httpd_remove_connection(httpd, &httpd->connections[0]);
    httpd->open_connections++;
    httpd->connections[0].socket_fd = fd;
    httpd->connections[0].connected = 1;
    httpd->connections[0].user_data = httpd->callbacks.conn_init(httpd->callbacks.opaque, local, local_len, remote, remote_len);
    return;
  }
  //added end.
...
httpd_thread(void *arg)
{
  ...
  while (1) {
    ...
    for (i=0; i<httpd->max_connections; i++) {
      ...
      //added by teddy.
      if (httpd->callbacks.conn_should_terminate(httpd->connections[i].user_data)) {
        logger_log(httpd->logger, LOGGER_INFO, "Connection closed for socket %d", httpd->connections[i].socket_fd);
        httpd_remove_connection(httpd, &httpd->connections[i]);
        continue;
      }
      //added end.
