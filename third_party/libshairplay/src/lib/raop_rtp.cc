  ...
  const unsigned char *aeskey, const unsigned char *aesiv)
{
  ...
  raop_rtp = reinterpret_cast<raop_rtp_t*>(calloc(1, sizeof(raop_rtp_t)));
  if (!raop_rtp) {
    return NULL;
  }
...
{
  raop_rtp_t *raop_rtp = reinterpret_cast<raop_rtp_t*>(opaque);
...
{
  raop_rtp_t *raop_rtp = reinterpret_cast<raop_rtp_t*>(arg);
  ...
  while(1) {
    ...
    if (FD_ISSET(raop_rtp->csock, &rfds)) {
      ...
      if (packetlen >= 12) {
        ...
        while (ret != -2 && (audiobuf = raop_buffer_dequeue(raop_rtp->buffer, &audiobuflen, no_resend))) {
          ret = raop_rtp->callbacks.audio_process(raop_rtp->callbacks.cls, cb_data, audiobuf, audiobuflen);
        }
...
raop_rtp_set_metadata(raop_rtp_t *raop_rtp, const char *data, int datalen)
{
  ...
  metadata = (unsigned char*)malloc(datalen);
  assert(metadata);
...
raop_rtp_set_coverart(raop_rtp_t *raop_rtp, const char *data, int datalen)
{
  ...
  coverart = (unsigned char*)malloc(datalen);
  assert(coverart);
