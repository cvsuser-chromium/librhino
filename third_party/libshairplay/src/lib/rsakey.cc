            ...
            const unsigned char *qInv, int qInv_len)
{
	...
	rsakey = (rsakey_t*)calloc(1, sizeof(rsakey_t));
	if (!rsakey) {
		return NULL;
	}
	rsakey->base64 = shairplay::base64_init(NULL, 0, 0);
	if (!rsakey->base64) {
		free(rsakey);
		return NULL;
	}
...
rsakey_destroy(rsakey_t *rsakey)
{
	if (rsakey) {
		...
		shairplay::base64_destroy(rsakey->base64);
		free(rsakey);
	}
}
            ...
            unsigned char *hwaddr, int hwaddrlen)
{
	...
	if (dstlen < shairplay::base64_encoded_length(rsakey->base64, rsakey->keylen)) {
		return -1;
	}

	/* Decode the base64 digest */
	digestlen = shairplay::base64_decode(rsakey->base64, &digest, b64digest, strlen(b64digest));
	if (digestlen < 0) {
		return -2;
	}
	...
	shairplay::base64_encode(rsakey->base64, dst, buffer, rsakey->keylen);
...
rsakey_mfg1(unsigned char *dst, int dstlen, const unsigned char *seed, int seedlen, int masklen)
{
	...
	for (i=0; i<iterations; i++) {
		...
		shairplay::SHA1_Init(&sha_ctx);
		shairplay::SHA1_Update(&sha_ctx, seed, seedlen);
		shairplay::SHA1_Update(&sha_ctx, counter, sizeof(counter));
		shairplay::SHA1_Final(dst+dstpos, &sha_ctx);
		dstpos += SHA1_SIZE;
	}
	return masklen;
}
...
rsakey_decrypt(rsakey_t *rsakey, unsigned char *dst, int dstlen, const char *b64input)
{
	...
	memset(buffer, 0, sizeof(buffer));
	inputlen = shairplay::base64_decode(rsakey->base64, &input, b64input, strlen(b64input));
...
rsakey_parseiv(rsakey_t *rsakey, unsigned char *dst, int dstlen, const char *b64input)
{
	...
	length = shairplay::base64_decode(rsakey->base64, &tmpptr, b64input, strlen(b64input));
	if (length < 0) {
		return -1;
