                                                    ...
                                                    void *context)
{
  ...
  if (errorCode == kDNSServiceErr_NoError)
  {
    ...
    if (flags & kDNSServiceFlagsAdd) {
      ...
      LOG(INFO) << "ZeroconfBrowserMDNS::BrowserCallback found service named: " << s.GetName()
                                                                    << "type: " << s.GetType()
                                                                  << "domain: " << s.GetDomain();
      p_this->addDiscoveredService(browser, s);
    } else {
      LOG(INFO) << "ZeroconfBrowserMDNS::BrowserCallback service named: " << s.GetDomain()
                             << " type: " << s.GetType()
                             << " domain: " << s.GetDomain();
  ...
  {
    LOG(ERROR_REPORT) << "ZeroconfBrowserMDNS::BrowserCallback returned, error = \n" << errorCode;
  }
}
                                                         ...
                                                         )
{
  ...
  if (errorCode) {
    LOG(ERROR_REPORT) << "ZeroconfBrowserMDNS: GetAddrInfoCallback failed with error = " << errorCode;
    return;
  }
                                                    ...
                                                    )
{
  ...
  if (errorCode) {
    LOG(ERROR_REPORT) << "ZeroconfBrowserMDNS: ResolveCallback failed with error = %ld" << errorCode;
    return;
  }
...
bool CZeroconfBrowserMDNS::doAddServiceType(const std::string& fcr_service_type)
{
  ...
  if(m_browser == NULL)
  {
    ...
    if (err != kDNSServiceErr_NoError) {
      LOG(ERROR_REPORT) << "ZeroconfBrowserMDNS: DNSServiceCreateConnection failed with error = " << err;
      return false;
    }
  }
  ...
  if( err != kDNSServiceErr_NoError )
  {
    ...
    LOG(ERROR_REPORT) << "ZeroconfBrowserMDNS: DNSServiceBrowse returned, error = " << err;
    return false;
  }
...
bool CZeroconfBrowserMDNS::doResolveService(CZeroconfBrowser::ZeroconfService& fr_service, double f_timeout)
{
  ...
  err = DNSServiceResolve(&sdRef, 0, kDNSServiceInterfaceIndexAny, fr_service.GetName().c_str(),
    fr_service.GetType().c_str(), fr_service.GetDomain().c_str(), ResolveCallback, this);
  ...
  if( err != kDNSServiceErr_NoError )
  {
    ...
    LOG(ERROR_REPORT) << "ZeroconfBrowserMDNS: DNSServiceResolve returned error = " << err;
    return false;
  }
  ...
  if (err != kDNSServiceErr_NoError) {
    LOG(ERROR_REPORT) << "ZeroconfBrowserMDNS::doResolveService DNSServiceProcessResult returned, error = " << err;
  }
  ...
  if (!fr_service.GetHostname().empty())
  {
    ...
    err = DNSServiceGetAddrInfo(&sdRef, 0, kDNSServiceInterfaceIndexAny,
      kDNSServiceProtocol_IPv4, fr_service.GetHostname().c_str(), GetAddrInfoCallback, this);

    if (err != kDNSServiceErr_NoError) {
      LOG(ERROR_REPORT) << "ZeroconfBrowserMDNS: DNSServiceGetAddrInfo returned, error = " << err;
    }
    ...
    {
      LOG(WARNING) << "ZeroconfBrowserMDNS: Could not resolve hostname %s " << fr_service.GetHostname()
                   << "falling back to CDNSNameCache";
      if (CDNSNameCache::Lookup(fr_service.GetHostname(), strIP))
        fr_service.SetIP(strIP);
      else
        LOG(ERROR_REPORT) << "ZeroconfBrowserMDNS: Could not resolve hostname %s" << fr_service.GetHostname();
    }
  }
...
void CZeroconfBrowserMDNS::ProcessResults()
{
  ...
  if (err != kDNSServiceErr_NoError)
    LOG(ERROR_REPORT) << "ZeroconfWIN: DNSServiceProcessResult returned, error = " << err;
}
