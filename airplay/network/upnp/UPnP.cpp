    ...
    m_CtrlPointHolder(new CCtrlPointReferenceHolder())
{
    ...
    if (CApplication::getInstance().getNetwork().GetFirstConnectedInterface()) {
      m_IP = CApplication::getInstance().getNetwork().GetFirstConnectedInterface()->GetCurrentIPAddress().c_str();
    }
...
CUPnP::MarkWatched(const CFileItem& item, const bool watched)
{
    if (upnp && upnp->m_MediaBrowser) {
        ...
        CMediaBrowser* browser = reinterpret_cast<CMediaBrowser*>(upnp->m_MediaBrowser);
