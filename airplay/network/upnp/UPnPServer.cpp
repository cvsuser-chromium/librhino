...
CUPnPServer::CUPnPServer(const char* friendly_name, const char* uuid /*= NULL*/, int port /*= 0*/) :
    ...
    m_scanning(CApplication::getInstance().IsMusicScanning() || CApplication::getInstance().IsVideoScanning())
{
}
...
+---------------------------------------------------------------------*/
void
CUPnPServer::Announce(AnnouncementFlag flag, const char *sender, const char *message, const std::string& json_data)
{
#if 1
#else
    NPT_String path;
...
static NPT_String TranslateWMPObjectId(NPT_String id)
{
    ...
    LOG(INFO) << "UPnP Translated id to " << id.GetChars();
    return id;
}
                              ...
                              const PLT_HttpRequestContext& context)
{
    ...
    LOG(INFO) << "Received UPnP Browse Metadata request for object /" << object_id;
#if 1
#else
                                    ...
                                    const PLT_HttpRequestContext& context)
{
    ...
    LOG(INFO) << "UPnP: Received Browse DirectChildren request for object '" << object_id << "', with sort criteria " << sort_criteria;
#if 1
    return NPT_FAILURE;
#else
                           ...
                           const char*                   parent_id /* = NULL */)
{
    ...
    LOG(INFO) << "Building UPnP response with filter '" << filter <<  "', starting @ " << starting_index << " with "
...
CUPnPServer::OnSearchContainer(PLT_ActionReference&          action,
                               ...
                               const PLT_HttpRequestContext& context)
{
    LOG(INFO) << "Received Search request for object '" << object_id
                                   << "' with search /" << search_criteria;
#if 1
#else
