...
//  result.Format("%02X:%02X:%02X:%02X:%02X:%02X", mAddr[0], mAddr[1], mAddr[2], mAddr[3], mAddr[4], mAddr[5]);
  base::StringAppendF(&result, "%02X:%02X:%02X:%02X:%02X:%02X", mAddr[0], mAddr[1], mAddr[2], mAddr[3], mAddr[4], mAddr[5]);
...
std::string CNetworkInterfaceWin32::GetCurrentWirelessEssId(void)
{
  ...
  if(IsWireless())
  {
    ...
    if(WlanOpenHandle(1,NULL,&dwVersion, &hClientHdl) == ERROR_SUCCESS)
    {
        ...
        for(unsigned int i=0; i<ppInterfaceList->dwNumberOfItems;i++)
        {
          ...
          if( strGuid == strAdaptername)
          {
            if(WlanQueryInterface(hClientHdl,&ppInterfaceList->InterfaceInfo[i].InterfaceGuid,wlan_intf_opcode_current_connection, NULL, &dwSize, (PVOID*)&pAttributes, NULL ) == ERROR_SUCCESS)
            {
              ...
              OutputDebugString(FILE_PATH_LITERAL("Can't query wlan interface\n"));
          }
        }
      }
      ...
      OutputDebugString(FILE_PATH_LITERAL("Can't open wlan handle\n"));
  }
#endif
  return result;
}
...
//  m_netrefreshTimer.StartZero();
  ...
  if (GetAdaptersInfo(adapterInfo, &ulOutBufLen) == ERROR_BUFFER_OVERFLOW)
  {
    ...
    if (adapterInfo == NULL)
    {
      OutputDebugString(FILE_PATH_LITERAL("Error allocating memory needed to call GetAdaptersinfo\n"));
      return;
    }
  }
...
std::vector<std::string> CNetworkWin32::GetNameServers(void)
{
  ...
  if (pFixedInfo == NULL)
  {
    OutputDebugString(FILE_PATH_LITERAL("Error allocating memory needed to call GetNetworkParams\n"));
    return result;
  }
...
void CNetworkWin32::SetNameServers(std::vector<std::string> nameServers)
{
  return;
}
...
bool CNetworkWin32::PingHost(unsigned long host, unsigned int timeout_ms /* = 2000 */)
{
  ...
  if (lastErr != ERROR_SUCCESS && lastErr != IP_REQ_TIMED_OUT) {
    ...
    LOG(ERROR_REPORT) << "IcmpSendEcho failed - " << WUSysMsg(lastErr).c_str();
  }
...
bool CNetworkInterfaceWin32::GetHostMacAddress(unsigned long host, std::string& mac)
{
  ...
  if (dwRetVal == NO_ERROR)
  {
    if (PhysAddrLen == 6)
    {
      ...
      LOG(ERROR_REPORT) << "SendArp completed successfully, but mac address has length != 6 /%d" << PhysAddrLen;
    }
  } else {
    //CLog::Log(LOGERROR, "%s - SendArp failed with error (%d)", __FUNCTION__, dwRetVal);
    LOG(ERROR_REPORT) << "SendArp failed with error /" << dwRetVal;
  }
...
std::vector<NetworkAccessPoint> CNetworkInterfaceWin32::GetAccessPoints(void)
{
  ...
  if (dwResult != ERROR_SUCCESS || !wlan_handle) {
    ...
    LOG(ERROR_REPORT) << "Could not load the client WLAN API";
    return result;
  }
  ...
  if (dwResult != ERROR_SUCCESS || !interface_list)
  {
    ...
    LOG(ERROR_REPORT) << "Failed to get the list of interfaces";
    return result;
  }
  ...
  for (unsigned int i = 0; i < interface_list->dwNumberOfItems; ++i) {
    ...
    if (strGuid == strAdaptername) {
      ...
      for (unsigned int j = 0; j < bss_list->dwNumberOfItems; ++j)
      {
        ...
        base::StringAppendF(&macAddress, "%02x-%02x-%02x-%02x-%02x-%02x",
...
void CNetworkInterfaceWin32::GetSettings(NetworkAssignment& assignment, std::string& ipAddress, std::string& networkMask, 
...
void CNetworkInterfaceWin32::SetSettings(NetworkAssignment& assignment, std::string& ipAddress, std::string& networkMask, 
         std::string& defaultGateway, std::string& essId, std::string& key, EncMode& encryptionMode)
{
  return;
}

void CNetworkInterfaceWin32::WriteSettings(FILE* fw, NetworkAssignment assignment, std::string& ipAddress, std::string& networkMask, 
        std::string& defaultGateway, std::string& essId, std::string& key, EncMode& encryptionMode)
{
  return;
}
