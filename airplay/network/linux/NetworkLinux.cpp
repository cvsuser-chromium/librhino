...

{
   ...
   m_interfaceMacAdr.clear();

//   m_interfaceMacAdr.Format("%02X:%02X:%02X:%02X:%02X:%02X",
   base::StringAppendF(&m_interfaceMacAdr, "%02X:%02X:%02X:%02X:%02X:%02X",
...
std::string CNetworkInterfaceLinux::GetCurrentDefaultGateway(void)
{
  ...
  if (pipe)
  {
    ...
    else
    {
      ...
      LOG(WARNING) << "Unable to determine gateway";
    }
    pclose(pipe);
  }
#elif defined(TARGET_FREEBSD)
...
CNetworkInterface* CNetworkLinux::GetFirstConnectedInterface(void)
{
	...
	LOG(INFO) << __FUNCTION__ << " no connected interface found - requery list";
        queryInterfaceList();
...
void CNetworkLinux::GetMacAddress(std::string interfaceName, char rawMac[6])
{
  memset(rawMac, 0, 6);
#if defined(TARGET_DARWIN) || defined(TARGET_FREEBSD)
...
#define IFT_ETHER 0x6/* Ethernet CSMACD */
  ...
  {
    if(std::string(interface->ifa_name).Equals(interfaceName))
    {
...
std::vector<std::string> CNetworkLinux::GetNameServers(void)
{
  ...
  if (pipe)
  {
    ...
    if (fread(buffer, sizeof(char), sizeof(buffer), pipe) > 0 && !ferror(pipe))
    {
      ...
      LOG(WARNING) << "Unable to determine nameserver";
    }
    pclose(pipe);
  }
...
void CNetworkLinux::SetNameServers(std::vector<std::string> nameServers)
{
...
bool CNetworkLinux::PingHost(unsigned long remote_ip, unsigned int timeout_ms)
{
  ...
  if (result < 0 || result > 1) {
    LOG(ERROR_REPORT) << "Ping fail : status = " << status << ", errno = " << errno << ": " << cmd_line;
...
//  CLog::Log(LOGERROR, "%s - GetHostMacAddress/ioctl failed with errno (%d)", __FUNCTION__, errno);
  ...
  //mac.Format("%02X:%02X:%02X:%02X:%02X:%02X",
  mac.clear();
  base::StringAppendF(&mac, "%02X:%02X:%02X:%02X:%02X:%02X",
...
std::vector<NetworkAccessPoint> CNetworkInterfaceLinux::GetAccessPoints(void)
{
   ...
   {
      LOG(WARNING) << GetName() << "-8.16s  Driver has no Wireless Extension version information.";
   ...
   if (ioctl(m_network->GetSocket(), SIOCSIWSCAN, &iwr) < 0)
   {
      ...
      if (errno == EPERM) {
         ...
         LOG(WARNING) << "Cannot initiate wireless scan: ioctl[SIOCSIWSCAN]: %s" << strerror(errno) << ". Try running as root";
      } else {
         //CLog::Log(LOGWARNING, "Cannot initiate wireless scan: ioctl[SIOCSIWSCAN]: %s", strerror(errno));
         LOG(WARNING) << "Cannot initiate wireless scan: ioctl[SIOCSIWSCAN]: %s" << strerror(errno);
      }
      return result;
   }
   ...
   while (duration < 15000)
   {
      ...
      if (res_buf == NULL)
      {
         ...
         LOG(WARNING) << "Cannot alloc memory for wireless scanning";
         return result;
      }
      ...
      if (errno == E2BIG && res_buf_len < 100000)
      {
         ...
         res_buf = NULL;
         res_buf_len *= 2;
         LOG(INFO) << "Scan results did not fit - trying larger buffer (" <<
                        (unsigned long) res_buf_len << " bytes)";
      ...
      else
      {
         ...
         LOG(WARNING) << "Cannot get wireless scan results: ioctl[SIOCGIWSCAN]: " << strerror(errno);
         free(res_buf);
         return result;
      }
   }
...
//            macAddress.Format("%02x-%02x-%02x-%02x-%02x-%02x", mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);
            base::StringAppendF(&macAddress, "%02x-%02x-%02x-%02x-%02x-%02x", mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);
         ...
         case SIOCGIWFREQ:
         {
            float freq = ((float)iwe->u.freq.m) * pow((float)10, iwe->u.freq.e);
            if (freq > 1000)
...
void CNetworkInterfaceLinux::SetSettings(NetworkAssignment& assignment, std::string& ipAddress, std::string& networkMask, std::string& defaultGateway, std::string& essId, std::string& key, EncMode& encryptionMode)
{
   ...
   if (system(cmd.c_str()) != 0) {
     ...
     LOG(ERROR_REPORT) << "Unable to stop interface " << GetName();
   } else {
     //CLog::Log(LOGINFO, "Stopped interface %s", GetName().c_str());
     LOG(ERROR_REPORT) << "Stopped interface " << GetName();
   }
   if (assignment != NETWORK_DISABLED)
   {
      ...
      if (system(cmd.c_str()) != 0) {
        ...
        LOG(ERROR_REPORT) << "Unable to start interface " << GetName();
      } else {
        //CLog::Log(LOGINFO, "Started interface %s", GetName().c_str());
        LOG(ERROR_REPORT) << "Started interface " << GetName();
      }
   }
#endif
}
