...
bool CDNSNameCache::Lookup(const std::string& strHostName, std::string& strIpAddress)
{
  ...
  strIpAddress.clear();//.Empty();

  if (address != INADDR_NONE)
  {
    //strIpAddress.Format("%d.%d.%d.%d", (address & 0xFF), (address & 0xFF00) >> 8, (address & 0xFF0000) >> 16, (address & 0xFF000000) >> 24 );
    base::SStringPrintf(&strIpAddress, "%d.%d.%d.%d", (address & 0xFF),
      (address & 0xFF00) >> 8, (address & 0xFF0000) >> 16, (address & 0xFF000000) >> 24 );
    return true;
  }
...
//  strIpAddress.Format("%d.%d.%d.%d", (unsigned char)host->h_addr_list[0][0], (unsigned char)host->h_addr_list[0][1], (unsigned char)host->h_addr_list[0][2], (unsigned char)host->h_addr_list[0][3]);
    base::SStringPrintf(&strIpAddress, "%d.%d.%d.%d", (unsigned char)host->h_addr_list[0][0], (unsigned char)host->h_addr_list[0][1], (unsigned char)host->h_addr_list[0][2], (unsigned char)host->h_addr_list[0][3]);
    g_DNSCache.Add(strHostName, strIpAddress);
    return true;
  }

  LOG(ERROR_REPORT) << "Unable to lookup host: " << strHostName.c_str();
  return false;
}
