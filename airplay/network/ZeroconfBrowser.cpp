...
bool CZeroconfBrowser::AddServiceType(const std::string& fcr_service_type /*const CStdString& domain*/ )
{
  CSingleLock lock(*mp_crit_sec);
...
std::vector<CZeroconfBrowser::ZeroconfService> CZeroconfBrowser::GetFoundServices()
{
  ...
  if(m_started) {
    ...
    LOG(INFO) << "CZeroconfBrowser::GetFoundServices asked for services without browser running";
...
void CZeroconfBrowser::ZeroconfService::SetType(const std::string& fcr_type)
{
  if(fcr_type.empty()){
    ...
    LOG(ERROR) << "CZeroconfBrowser::ZeroconfService::SetType invalid type: " << fcr_type;
  }
...
void CZeroconfBrowser::ZeroconfService::SetTxtRecords(const tTxtRecordMap& txt_records)
{
  ...
  LOG(INFO) << "CZeroconfBrowser: dump txt-records";
  for(tTxtRecordMap::const_iterator it = m_txtrecords_map.begin(); it != m_txtrecords_map.end(); ++it)
  {
    LOG(INFO) << "CZeroconfBrowser:  key: " << it->first.c_str()
...
{
  return std::string(fcr_service.m_type + "@" + fcr_service.m_domain + "@" + fcr_service.m_name);
}
...
CZeroconfBrowser::ZeroconfService CZeroconfBrowser::ZeroconfService::fromPath(const std::string& fcr_path)
{
  if( fcr_path.empty() ) {
    ...
    LOG(ERROR) << "CZeroconfBrowser::ZeroconfService::fromPath input string empty!" ;
  }
  ...
  if( pos1 == -1 || pos2 == -1 ) {
    ...
    LOG(INFO) << "CZeroconfBrowser::ZeroconfService::fromPath invalid input path" ;
  }
