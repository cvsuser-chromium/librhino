...
// A type declaration or definition uses a type qualifier
// (const, volatile, signed, or unsigned) more than once.
// This causes a warning with Microsoft extensions (/Ze)
// and an error under ANSI compatibility (/Za).
...
mDNSlocal void SendResponses(mDNS *const m)
{
    ...
    m->NextScheduledResponse = m->timenow + kNextScheduledResponse;
    ...
    while (intf)
    {
        ...
        if (m->omsg.h.numAnswers || m->omsg.h.numAdditionals)
        {
            ...
            if (intf->IPv4Available){
              ...
              mDNSSendDNSMessage(m, &m->omsg, responseptr, intf->InterfaceID, mDNSNULL, &AllLocalHost_v4, MulticastDNSPort, mDNSNULL, mDNSNULL, mDNSfalse);
            }
...
mDNSlocal void SendQueries(mDNS *const m)
{
    ...
    while (intf)
    {
        ...
        if (queryptr > m->omsg.data)
        {
            ...
            if (intf->IPv4Available) {
              mDNSSendDNSMessage(m, &m->omsg, queryptr, intf->InterfaceID, mDNSNULL, &AllDNSLinkGroup_v4, MulticastDNSPort, mDNSNULL, mDNSNULL, useBackgroundTrafficClass);
              mDNSSendDNSMessage(m, &m->omsg, queryptr, intf->InterfaceID, mDNSNULL, &AllLocalHost_v4, MulticastDNSPort, mDNSNULL, mDNSNULL, useBackgroundTrafficClass);
            }
            if (intf->IPv6Available)
              mDNSSendDNSMessage(m, &m->omsg, queryptr, intf->InterfaceID, mDNSNULL, &AllDNSLinkGroup_v6, MulticastDNSPort, mDNSNULL, mDNSNULL, useBackgroundTrafficClass);
            if (!m->SuppressSending)
              m->SuppressSending = NonZeroTime(m->timenow + (mDNSPlatformOneSecond+9)/10);
            if (++pktcount >= 1000) {
            ...
            else
            {
                ...
                if (*p)
                  *p = rr->next;                  // Cut this record from the list
                  ...
                  LogMsg("mDNS_Execute: ERROR!! Cannot find record %s in ResourceRecords list", ARDisplayString(m, rr)); break;
                }
                if (!head)
                {
                    while (*p) p=&(*p)->next;
...
mDNSlocal void BeginSleepProcessing(mDNS *const m)
{
    ...
    if (!mDNSOpaque64IsZero(&updateIntID))
      SPSInitRecordsBeforeUpdate(m, updateIntID);
                             ...
                             mDNSBool AdvertiseLocalAddresses, mDNSCallback *Callback, void *Context)
{
    ...
    m->NextScheduledResponse   = timenow + kNextScheduledResponse;
