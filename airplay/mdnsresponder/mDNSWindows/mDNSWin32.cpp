...
#include	"SystemService/Firewall.h"
#include	"RegNames.h"
#include	"Secret.h"
#include	<dns_sd.h>
	...
	)
{
	...
	sock->userCallback	= reinterpret_cast<TCPUserCallback>(inCallback);
...
mDNSexport long	mDNSPlatformReadTCP( TCPSocket *sock, void *inBuffer, unsigned long inBufferSize, mDNSBool * closed )
{
	...
	*closed = mDNSfalse;
    nread = recv( sock->fd, (char*)inBuffer, inBufferSize, 0 );
		...
		mDNSBool 					useBackgroundTrafficClass )
{
	...
	if( inDstIP->type == mDNSAddrType_IPv4 )
	{
		...
		if (inSrcSocket) { 
      sendingsocket = inSrcSocket->fd; 
      debugf("mDNSPlatformSendUDP using port %d, static port %d, sock %d", mDNSVal16(inSrcSocket->port), inMDNS->p->unicastSock4.fd, sendingsocket); 
    }
	}
...
SetDomainFromDHCP( void )
{
	...
	for ( pAdapter = pAdapterInfo; pAdapter; pAdapter = pAdapter->Next )
	{
        ...
        //added by teddy, at 2013/10/25 10:19:34
        //ignore the RegQueryString result.
        err = 0;
				check_noerr( err );
			}

			if ( domain && domain[0] ) 
        mDNS_AddSearchDomain_CString(domain, mDNSNULL);
