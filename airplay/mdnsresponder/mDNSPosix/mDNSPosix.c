    ...
    if (packetLen >= 0)
    {
        ...
        if ((destAddr.NotAnInteger == 0) && (flags & MSG_MCAST))
        {
            ...
            if (senderAddr.type == mDNSAddrType_IPv4)
              ...
              destAddr.ip.v6 = AllDNSLinkGroup_v6.ip.v6;
        }
        #endif
...
mDNSlocal int SetupSocket(struct sockaddr *intfAddr, mDNSIPPort port, int interfaceIndex, int *sktPtr)
{
    ...
    if (intfAddr->sa_family == AF_INET)
    {
        ...
        //let send broadcast in this socket.
        err = setsockopt(*sktPtr, SOL_SOCKET, SO_BROADCAST,&kOn,sizeof(kOn));
        if (err < 0) { err = errno; perror("setsockopt - SO_BROADCAST"); }
