...
//        (void)_snprintf(val, 64, "%llu", node_data->intval);
        base::snprintf(val, 64, "%llu", node_data->intval);
        break;
...
//        (void)_snprintf(val, 64, "%f", node_data->realval);
        base::snprintf(val, 64, "%f", node_data->realval);
...
static void xml_to_node(xmlNodePtr xml_node, plist_t * plist_node)
{
    ...
    for (node = xml_node->children; node; node = node->next)
    {
        ...
        if (!xmlStrcmp(node->name, XPLIST_INT))
        {
            ...
            //data->intval = _strtoui64((char*)strval, NULL, 0);
            base::StringToUint64((char*)strval, &data->intval);
            data->type = PLIST_UINT;
            data->length = 8;
            xmlFree(strval);
            continue;
        ...
        if (!xmlStrcmp(node->name, XPLIST_DATA))
        {
            ...
            base::Base64Decode((char*)strval, &output);
            unsigned char *dec = (unsigned char *)output.data();
            size = output.size();
